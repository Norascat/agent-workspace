...
4. Save the new skill to `.codex/skills/{name}/SKILL.md` (唯一位置，不再双写存档)
...
1. **Skills** — `.codex/skills/*/SKILL.md` — read each skill's name and description from frontmatter
2. **Workspace config** — `.codex/config.json` if present
3. **显式命令** — 从每个 skill 的 description 中提取工作区脚本入口，如 `bin/...`

### Output format

Write/update the file `02_codex/02_skills/skills.md`. 这是一个「命令/脚本 → 技能」映射清单，不是完整元数据表：
